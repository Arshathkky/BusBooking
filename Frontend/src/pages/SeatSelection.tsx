...
const SeatLayout: React.FC<SeatLayoutProps> = ({
  ...
  for (let i = 1; i <= totalSeats; i++) {
    ...
    let seatClass =
      "w-12 h-12 rounded-lg border-2 text-xs font-semibold transition-all duration-200 cursor-pointer flex flex-col items-center justify-center ";
    ...
    seats.push(
      ...
      >
        {isOccupied ? (
          ...
          <span className="text-[10px] font-semibold text-pink-700">L</span>
        ) : null}
    ...
    <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 relative transition-colors">
        ...
        {Array.from({ length: Math.ceil(totalSeats / 5) }, (_, rowIndex) => (
          <div key={rowIndex} className="flex items-center justify-center space-x-2">
            <div className="flex space-x-1">
              {[0, 1, 2].map((offset) => {
                const num = rowIndex * 5 + offset + 1;
                return num <= totalSeats ? seats[num - 1] : null;
              })}
            </div>
      ...
      <div className="flex justify-center mt-6 space-x-4 text-sm">
        <div className="flex items-center space-x-1">
          <div className="w-4 h-4 bg-[#fdc106] rounded"></div>
          <span>Selected</span>
        </div>
        <div className="flex items-center space-x-1">
...
const SeatSelection: React.FC = () => {
  ...
  if (!searchData) {
    return (
      <div className="text-center mt-8">
        <p className="text-gray-600">No search data found. Please search for buses first.</p>
        <button
          onClick={() => navigate("/")}
  ...
  const handleSeatClick = (seatNumber: number) => {
    ...
    if (selectedSeats.includes(seatNumber)) deselectSeat(seatNumber);
    else selectSeat(seatNumber);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center space-x-4 mb-8">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center space-x-2 text-[#fdc106] hover:text-[#e6ad05]"
        >
